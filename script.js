...
function initPlayer(id, handle, options) {
	...
	new MediaElement(id, {
		...
		success: async function (media, domNode) {
			...
			media.addEventListener('canplay', async () => {
				...
				if (media.youTubeApi) {
					...
					setTimeout(() => {
						...
						let button2 = media.twitchPlayer._iframe.contentWindow.document.querySelector('button[data-a-target="player-play-pause-button"]');
